...
export default function CampaignDetailsPage() {
  ...
  const SYS_LABELS = {
    dnd5e: "D&D 5E", pf2e: "PF 2E", coc7e: "CoC 7E",
    wh4e: "WH 4E", morkborg: "Mörk Borg", other: "Inny",
  };

  const isGM      = campaign?.owner;
  const roleLabel = isGM ? "Mistrz Gry" : (campaign?.myRole || "Gracz");
  const sysLabel  = SYS_LABELS[campaign?.systemCode] || campaign?.systemCode?.toUpperCase() || "RPG";

  return (
    <div className="page campaignDetailsPage">

      {/* Back */}
      <Link to="/campaigns" className="cdBackBtn">
      ...
      {!loading && !error && campaign && (
        <>
          ...
          <section className="cdHero">
            <div className="cdHeroCover" style={coverStyle}>
              {!campaign.coverImageUrl && (
                <div className="cdHeroCoverIcon">
            ...
            <div className="cdHeroBody">
              <div className="cdHeroTop">
                <div className="cdHeroBadges">
                  <span className="cdBadgeSys">{sysLabel}</span>
                  <span className={isGM ? "cdBadgeGM" : "cdBadgePlayer"}>
                    {isGM ? "MG" : roleLabel}
                  </span>
                  {campaign.status === "ACTIVE" && (
                    <span className="cdBadgeActive">Aktywna</span>
                  )}
                </div>
                <h1 className="cdHeroTitle">{campaign.title}</h1>
                {campaign.description && (
                  <p className="cdHeroDesc">{campaign.description}</p>
                )}
              </div>
              <div className="cdHeroMeta">
                <div className="cdMetaItem">
                  <div className="cdMetaLabel">System</div>
                  <div className="cdMetaValue">{sysLabel}</div>
                </div>
                <div className="cdMetaItem">
                  <div className="cdMetaLabel">Twoja rola</div>
                  <div className="cdMetaValue">{roleLabel}</div>
                </div>
                <div className="cdMetaItem">
                  <div className="cdMetaLabel">Członkowie</div>
                  <div className="cdMetaValue">{members.length || "…"}</div>
                </div>
                <div className="cdMetaItem">
                  <div className="cdMetaLabel">Sesje</div>
                  <div className="cdMetaValue">{sessions.length}</div>
                </div>
                {isGM && campaign.inviteCode && (
                  <div className="cdMetaItem">
                    <div className="cdMetaLabel">Kod zaproszenia</div>
                    <div className="cdMetaValue cdMetaCode">{campaign.inviteCode}</div>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* ── Tabs ── */}
          <nav className="cdTabNav" aria-label="Nawigacja kampanii">
            {visibleTabs.map((tab) => (
              <button
                key={tab.id}
                type="button"
                className={`cdTabBtn${activeTab === tab.id ? " is-active" : ""}`}
                onClick={() => setActiveTab(tab.id)}
              >
                {tab.label}
                {tab.id === "members"  && members.length  > 0 && <span className="cdTabCount">{members.length}</span>}
                {tab.id === "sessions" && sessions.length > 0 && <span className="cdTabCount">{sessions.length}</span>}
              </button>
            ))}
          </nav>
