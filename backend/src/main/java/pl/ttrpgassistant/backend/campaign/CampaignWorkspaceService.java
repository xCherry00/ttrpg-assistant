...
public class CampaignWorkspaceService {
    ...
    public CampaignSummaryResponse updateCampaign(Long userId, Long campaignId, UpdateCampaignRequest request) {
        ...
        return new CampaignSummaryResponse(
                ...
                "GM",
                true,
                normalizeVisibility(saved.getVisibility(), "PRIVATE"),
                normalizePlayerLimit(saved.getPlayerLimit(), 5),
                campaignMemberRepository.countPlayersByCampaignId(saved.getId(), saved.getOwnerUserId()),
                displayNameFor(getUser(saved.getOwnerUserId())),
                campaignFavoriteRepository.existsById(new CampaignFavoriteId(userId, saved.getId())),
                List.of(),
                null,
                null,
                saved.getCreatedAt(),
    ...
    private String normalizeVisibility(String rawValue, String fallback) {
        if (rawValue == null || rawValue.isBlank()) {
            return fallback == null || fallback.isBlank() ? "PRIVATE" : fallback.trim().toUpperCase(Locale.ROOT);
        }
        String normalized = rawValue.trim().toUpperCase(Locale.ROOT);
        return "PUBLIC".equals(normalized) ? "PUBLIC" : "PRIVATE";
    }

    private int normalizePlayerLimit(Integer rawValue, Integer fallback) {
        int value = rawValue == null ? (fallback == null ? 5 : fallback) : rawValue;
