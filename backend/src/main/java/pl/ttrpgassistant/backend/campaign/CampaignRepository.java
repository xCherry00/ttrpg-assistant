...
public interface CampaignRepository extends JpaRepository<CampaignEntity, Long> {
    ...
    @Query(value = """
            ...
            from campaigns c
            where c.deleted_at is null
              and upper(c.visibility) = 'PUBLIC'
              and not exists (
                  select 1
                  from campaign_members current_member
                  where current_member.campaign_id = c.id
                    and current_member.user_id = :userId
              )
              and (
                  select count(*)
                  from campaign_members member_count
                  where member_count.campaign_id = c.id
                    and member_count.user_id <> c.owner_user_id
              ) < c.player_limit
            order by c.updated_at desc, c.created_at desc
            """, nativeQuery = true)
    List<CampaignEntity> findJoinablePublicCampaigns(@Param("userId") Long userId);
